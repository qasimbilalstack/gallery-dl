...
def parse_command_line(module, argv):
    ...
    if getattr(opts, "wait_for_video", None) is not None:
        ...
        opts.wait_for_video = (
            module.parse_duration(min_wait),
    ...
    if ytdlp:
        ...
        if isinstance(parse_metadata, dict):
            ...
            opts.parse_metadata = {
                k: list(itertools.chain.from_iterable(map(metadataparser_actions, v)))
            ...
            opts.parse_metadata = list(
                itertools.chain.from_iterable(
                    map(metadataparser_actions, parse_metadata)
                )
            )
    ...
    download_archive_fn = (
        module.expand_path(opts.download_archive)
    ...
    if getattr(opts, "no_sponsorblock", None):
        ...
        opts.sponsorblock_mark = getattr(opts, "sponsorblock_mark", None) or set()
        opts.sponsorblock_remove = getattr(opts, "sponsorblock_remove", None) or set()
    opts.remove_chapters = getattr(opts, "remove_chapters", None) or ()

    try:
        postprocessors = list(module.get_postprocessors(opts))
    except AttributeError:
        postprocessors = legacy_postprocessors(opts, module, ytdlp, compat_opts)
    ...
    return {
        ...
        "format": opts.format,
        "allow_unplayable_formats": getattr(opts, "allow_unplayable_formats", None),
        "ignore_no_formats_error": getattr(opts, "ignore_no_formats_error", None),
        "format_sort": getattr(opts, "format_sort", None),
        "format_sort_force": getattr(opts, "format_sort_force", None),
        "allow_multiple_video_streams": opts.allow_multiple_video_streams,
        "allow_multiple_audio_streams": opts.allow_multiple_audio_streams,
        "check_formats": getattr(opts, "check_formats", None),
        "outtmpl": opts.outtmpl,
        ...
        "write_all_thumbnails": getattr(opts, "write_all_thumbnails", None)
        or opts.writethumbnail == "all",
        ...
        "skip_playlist_after_errors": getattr(opts, "skip_playlist_after_errors", None),
        ...
        "sleep_interval_requests": getattr(opts, "sleep_interval_requests", None),
        "sleep_interval": opts.sleep_interval,
        "max_sleep_interval": opts.max_sleep_interval,
        "sleep_interval_subtitles": getattr(opts, "sleep_interval_subtitles", None),
        ...
        "hls_split_discontinuity": getattr(opts, "hls_split_discontinuity", None),
        "external_downloader_args": opts.external_downloader_args,
        "postprocessor_args": opts.postprocessor_args,
        "cn_verification_proxy": opts.cn_verification_proxy,
        "geo_verification_proxy": opts.geo_verification_proxy,
        "geo_bypass": getattr(opts, "geo_bypass", "default"),
        "geo_bypass_country": getattr(opts, "geo_bypass_country", None),
        "geo_bypass_ip_block": getattr(opts, "geo_bypass_ip_block", None),
...
def legacy_postprocessors(opts, module, ytdlp, compat_opts):
    ...
    if opts.convertsubtitles:
        pp = {"key": "FFmpegSubtitlesConvertor", "format": opts.convertsubtitles}
        if ytdlp:
            pp["when"] = "before_dl"
    ...
    if opts.embedthumbnail:
        already_have_thumbnail = opts.writethumbnail or getattr(
