...
class UgoiraAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, value, option_string=None):
        ...
        if value in ("webm", "vp9"):
            pp = {
                "extension": "webm",
                "ffmpeg-args": ("-c:v", "libvpx-vp9", "-crf", "12", "-b:v", "0", "-an"),
            }
...
def build_parser():
    ...
    if util.EXECUTABLE:
        ...
        update.add_argument(
            ...
            metavar="CHANNEL[@TAG]",
            help=(
                "Switch to a dfferent release channel (stable or dev) "
    ...
    output.add_argument(
        ...
        "--print",
        dest="postprocessors",
        metavar="[EVENT:]FORMAT",
        action=PrintAction,
        const="-",
        default=[],
        help=(
            "Write FORMAT during EVENT (default 'prepare') to standard "
    ...
    output.add_argument(
        ...
        nargs=2,
        help=(
            "Append FORMAT during EVENT to FILE instead of downloading "
    ...
    output.add_argument(
        "--write-unsupported",
        dest="unsupportedfile",
        metavar="FILE",
        action=ConfigAction,
        help=(
            "Write URLs, which get emitted by other extractors but cannot "
            "be handled, to FILE"
        ),
    )
    output.add_argument(
    ...
    downloader.add_argument(
        ...
        action=ConfigAction,
        help=(
            "Number of seconds to wait before each download. "
    ...
    downloader.add_argument(
        ...
        action=ConfigAction,
        help=(
            "Number of seconds to wait when receiving a "
            "'429 Too Many Requests' response"
        ),
    )
    downloader.add_argument(
        "--sleep-extractor",
        dest="sleep-extractor",
        metavar="SECONDS",
        action=ConfigAction,
        help=(
            "Number of seconds to wait before starting data extraction "
            "for an input URL"
        ),
    )
    downloader.add_argument(
    ...
    downloader.add_argument(
        ...
        const=False,
        help=(
            "Do not set file modification times according to "
    ...
    configuration.add_argument(
        ...
        default=[],
        help=("Additional options. " "Example: -o browser=firefox"),
    )
    configuration.add_argument(
        "-c",
        "--config",
    ...
    cookies.add_argument(
        ...
        help=(
            "Name of the browser to load cookies from, with optional "
    ...
    selection.add_argument(
        ...
        metavar="N",
        help=(
            "Stop current & parent extractors "
    ...
    selection.add_argument(
        ...
        action=ConfigAction,
        help=(
            "Record successfully downloaded files in FILE and "
    ...
    postprocessor.add_argument(
        ...
        const=0,
        help=(
            "Rename previously downloaded files from FORMAT "
    ...
    postprocessor.add_argument(
        ...
        const=1,
        help=(
            "Rename previously downloaded files from the current filename "
            "format to FORMAT"
        ),
    )
    postprocessor.add_argument(
        ...
        action=UgoiraAction,
        help=(
            "Convert Pixiv Ugoira to FMT using FFmpeg. "
    ...
    postprocessor.add_argument(
        ...
        dest="postprocessors",
        metavar="CMD",
        action=AppendCommandAction,
        const={"name": "exec"},
        help=(
            "Execute CMD for each downloaded file. "
    ...
    postprocessor.add_argument(
        ...
        metavar="CMD",
        action=AppendCommandAction,
        const={"name": "exec", "event": "finalize"},
        help=(
            "Execute CMD after all files were downloaded. "
