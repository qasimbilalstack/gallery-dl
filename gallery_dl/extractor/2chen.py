...
class _2chenThreadExtractor(Extractor):
    ...
    def items(self):
        ...
        for post in self.posts(page):
            ...
            yield Message.Url, url, text.nameext_from_url(post["filename"], post)
    ...
    def posts(self, page):
        ...
        return map(
            self.parse, text.extract_iter(page, 'class="glass media', "</article>")
        )
    ...
    def parse(self, post):
        ...
        return {
            ...
            "date": text.parse_datetime(
                extr("<time", "<").partition(">")[2], "%d %b %Y (%a) %H:%M:%S"
            ),
            "no": extr('href="#p', '"'),
            "url": extr('</a><a href="', '"'),
...
class _2chenBoardExtractor(Extractor):
    ...
    def items(self):
        ...
        for thread in text.extract_iter(page, '<figure><a href="', '"'):
