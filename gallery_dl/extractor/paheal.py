...
class PahealExtractor(Extractor):
    ...
    def items(self):
        self.cookies.set("ui-tnc-agreed", "true", domain="rule34.paheal.net")
    ...
    def _extract_post(self, post_id):
        ...
        post = {
            ...
            "md5": extr("/_thumbs/", "/"),
            "file_url": (
                extr("id='main_image' src='", "'") or extr("<source src='", "'")
            ),
            "uploader": text.unquote(extr("class='username' href='/user/", "'")),
            "date": text.parse_datetime(extr("datetime='", "'"), "%Y-%m-%dT%H:%M:%S%z"),
            "source": text.unescape(
                text.extr(extr(">Source Link<", "</td>"), "href='", "'")
            ),
        }
...
class PahealTagExtractor(PahealExtractor):
    ...
    pattern = (
        r"(?:https?://)?(?:rule34|rule63|cosplay)\.paheal\.net" r"/post/list/([^/?#]+)"
    )
    ...
    def get_posts(self):
        ...
        while True:
            ...
            for post in text.extract_iter(page, "<img id='thumb_", "Only</a>", pos):
...
class PahealPostExtractor(PahealExtractor):
    ...
    subcategory = "post"
    pattern = (
        r"(?:https?://)?(?:rule34|rule63|cosplay)\.paheal\.net" r"/post/view/(\d+)"
    )
