...
class TsuminoGalleryExtractor(TsuminoBase, GalleryExtractor):
    ...
    def metadata(self, page):
        ...
        return {
            ...
            "rating": text.parse_float(
                extr('id="Rating">', "</div>").partition(" ")[0]
            ),
    ...
    def images(self, page):
        ...
        return [
            (base + str(i) + params, None) for i in range(1, text.parse_int(cnt) + 1)
        ]
...
class TsuminoSearchExtractor(TsuminoBase, Extractor):
    ...
    def _parse(self, query):
        ...
        try:
            ...
            raise exception.AbortExtraction(f"Invalid search query '{query}' ({exc})")
