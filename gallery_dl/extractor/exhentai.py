...
class ExhentaiGalleryExtractor(ExhentaiExtractor):
    ...
    subcategory = "gallery"
    pattern = (
        BASE_PATTERN + r"(?:/g/(\d+)/([\da-f]{10})" r"|/s/([\da-f]{10})/(\d+)-(\d+))"
    )
    ...
    def items(self):
        ...
        if self.gallery_token:
            ...
            if not self.image_token:
                ...
                raise exception.AbortExtraction("Failed to extract initial image token")
            ...
            if not part:
                self.log.debug("Page content:\n%s", ipage)
                raise exception.AbortExtraction("Failed to extract gallery token")
    ...
    def image_from_page(self, page):
        ...
        try:
            ...
            self.log.debug("Page content:\n%s", page)
            raise exception.AbortExtraction(f"Unable to parse image info for '{url}'")
    ...
    def _validate_response(self, response):
        if response.history or not response.headers.get("content-type", "").startswith(
            "text/html"
        ):
            return True
    ...
    def _validate_signature(self, signature):
        ...
        if signature:
            if byte := signature[0]:
                # 60 == b"<"
                if byte == 60 and b"<!doctype html".startswith(signature[:14].lower()):
    ...
    def _image_page(self):
        url = f"{self.root}/s/{self.image_token}" f"/{self.gallery_id}-{self.image_num}"
