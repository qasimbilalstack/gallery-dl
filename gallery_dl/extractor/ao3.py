...
BASE_PATTERN = r"(?:https?://)?(?:www\.)?" r"a(?:rchiveofourown|o3)\.(?:org|com|net)"
...
class Ao3Extractor(Extractor):
    ...
    def _login_impl(self, username, password):
        ...
        token = text.extract(page, ' name="authenticity_token" value="', '"', pos)[0]
        if not token:
    ...
    def _pagination(self, path, needle='<li id="work_'):
        while True:
            ...
            path = text.extr(page, '<a rel="next" href="', '"') or text.extr(
                page, '<li class="next"><a href="', '"'
            )
            if not path:
                return
...
class Ao3WorkExtractor(Ao3Extractor):
    ...
    def items(self):
        ...
        if (
            len(page) < 20000
            and '<h2 class="landmark heading">Adult Content Warning</' in page
        ):
        ...
        data = {
            ...
            "rating": text.split_html(extr('<dd class="rating tags">', "</dd>")),
            "warnings": text.split_html(extr('<dd class="warning tags">', "</dd>")),
            "categories": text.split_html(extr('<dd class="category tags">', "</dd>")),
            "fandom": text.split_html(extr('<dd class="fandom tags">', "</dd>")),
            "relationships": text.split_html(
                extr('<dd class="relationship tags">', "</dd>")
            ),
            "characters": text.split_html(extr('<dd class="character tags">', "</dd>")),
            "tags": text.split_html(extr('<dd class="freeform tags">', "</dd>")),
            ...
            "date_updated": text.parse_timestamp(path.rpartition("updated_at=")[2]),
            "words": text.parse_int(extr('<dd class="words">', "<").replace(",", "")),
            "chapters": chapters,
            "comments": text.parse_int(
                extr('<dd class="comments">', "<").replace(",", "")
            ),
            "likes": text.parse_int(extr('<dd class="kudos">', "<").replace(",", "")),
            "bookmarks": text.parse_int(
                text.remove_html(extr('<dd class="bookmarks">', "</dd>")).replace(
                    ",", ""
                )
            ),
            "views": text.parse_int(extr('<dd class="hits">', "<").replace(",", "")),
            "title": text.unescape(
                text.remove_html(extr(' class="title heading">', "</h2>")).strip()
            ),
            "author": text.unescape(
                text.remove_html(extr(' class="byline heading">', "</h3>"))
            ),
...
class Ao3UserWorksExtractor(Ao3Extractor):
    ...
    pattern = (
        BASE_PATTERN + r"(/users/([^/?#]+)/(?:pseuds/([^/?#]+)/)?" r"works(?:/?\?.+)?)"
    )
...
class Ao3UserSeriesExtractor(Ao3Extractor):
    ...
    pattern = (
        BASE_PATTERN + r"(/users/([^/?#]+)/(?:pseuds/([^/?#]+)/)?" r"series(?:/?\?.+)?)"
    )
