    ...
    def items(self):
        ...
        for info in self._pagination(page):
            ...
            item = {
                ...
                "_extractor": (
                    PixnetFolderExtractor if "/folder/" in url else PixnetSetExtractor
                ),
            }
            item.update(data)
...
class PixnetImageExtractor(PixnetExtractor):
    ...
    def items(self):
        ...
        params = {
            "url": (f"https://{self.blog}.pixnet.net" f"/album/photo/{self.item_id}"),
            "format": "json",
        }

        data = self.request_json(url, params=params)
        data["id"] = text.parse_int(data["url"].rpartition("/")[2].partition("-")[0])
