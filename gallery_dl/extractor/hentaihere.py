...
class HentaihereMangaExtractor(HentaihereBase, MangaExtractor):
    ...
    def chapters(self, page):
        ...
        manga, pos = text.extract(page, '<span itemprop="name">', "</span>", pos)
        mtype, pos = text.extract(
            page, '<span class="mngType text-danger">[', "]</span>", pos
        )
        manga_id = text.parse_int(self.page_url.rstrip("/").rpartition("/")[2][1:])

        while True:
            marker, pos = text.extract(page, '<li class="sub-chp clearfix">', "", pos)
            if marker is None:
                return results
