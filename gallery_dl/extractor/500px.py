...
class _500pxExtractor(Extractor):
    ...
    def _extend(self, edges):
        ...
        return [
            photos[pid]
            for pid in ids
            if pid in photos or self.log.warning("Unable to fetch photo %s", pid)
        ]

    def _request_api(self, url, params):
        headers = {
            "Origin": self.root,
            "x-csrf-token": self.cookies.get("x-csrf-token", domain=".500px.com"),
        }
...
class _500pxGalleryExtractor(_500pxExtractor):
    ...
    pattern = BASE_PATTERN + r"/(?!photo/)(?:p/)?" r"([^/?#]+)/galleries/([^/?#]+)"
