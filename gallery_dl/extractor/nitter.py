...
class NitterExtractor(BaseExtractor):
    ...
    def _tweet_from_html(self, html):
        ...
        return {
            ...
            "comments": text.parse_int(
                extr('class="icon-comment', "</div>").rpartition(">")[2]
            ),
            "retweets": text.parse_int(
                extr('class="icon-retweet', "</div>").rpartition(">")[2]
            ),
            "quotes": text.parse_int(
                extr('class="icon-quote', "</div>").rpartition(">")[2]
            ),
            "likes": text.parse_int(
                extr('class="icon-heart', "</div>").rpartition(">")[2]
            ),
    ...
    def _user_from_html(self, html):
        ...
        try:
            if "/enc/" in banner:
                uid = (
                    binascii.a2b_base64(banner.rpartition("/")[2])
                    .decode()
                    .split("/")[4]
                )
            else:
        ...
        return {
            ...
            "profile_image": self.root
            + extr('class="profile-card-avatar" href="', '"'),
            "nick": extr('title="', '"'),
            "name": extr('title="@', '"'),
